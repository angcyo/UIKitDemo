...
class StickHeaderOldBehavior(
    ...
    fun _offsetTop() {
        _childView?.offsetTopTo(_currentOffsetTop)
        _pagerWrapView?.offsetTopTo(_childView!!.bottom)
        _currentOffsetTop = _childView?.top ?: 0
    }
    ...
    fun moveLayout(dy: Int): Boolean {
        ...
        _childView!!.offsetTop(-dy, -_childView!!.measuredHeight, 0)
        _pagerWrapView?.offsetTopTo(_childView!!.bottom)
        _currentOffsetTop = _childView!!.top

        result = (dy < 0 && _childView!!.top == 0) || (dy > 0 && _childView!!.bottom == 0)
